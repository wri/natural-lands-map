...
//// MAPBIOMAS--------------------------------------------------------------------------
/// PROCESSING OF MAPBIOMAS WERE DONE IN A DIFFERENT SCRIPT: see naturalLands_1_preProcessing_Mapbiomas
// Brazil collection 8.0 (2020)
// Amazon collection 5.0 (2020)
// Chaco collection 4.0 (2020)
// Atlantic forest collection 3.0 (2020)
// Pampa collection 3.0 (2020)
// Indonesia collection 2.0 (2020)
// Peru collection 2.0 (2020)
// Bolivia collection 1.0 (2020)
// Colombia collection 1.0 (2020)
// Venezuela collection 1.0 (2020)
// Uruguay collection 1.0 (2020)
// Ecuador collection 1.0 (2020)
...
// South America
var mapbiomas_sa = ee.Image('projects/wri-datalab/SBTN/map/mapbiomas_southAmerica_remapped_v1_20240806').rename(['constant']).selfMask();  
...
//mixed short veg/forest classes are temporarily remapped to 26 
...
// Overlay UMD tree height data with SANLC mixed short veg/forest classes (remapped as 26)                            
...
var nlcd2019 = nlcd.filter(ee.Filter.eq('system:index', '2019')).first().select('landcover')
                      .rename(['constant']).reproject({crs:umdProj});
// EXPORT THIS FOR FASTER PROCESSING ^
...
// therefore we used the Gridded Livestock of the World 4.0 2020 data to create a threshold of 
// livestock density as a proxy for intensive pasture. See technical note for explanation 
// of how thresholds were decided.

//// Gridded Livestock of the World 4.0 2020--------------------------------------------------------------------------
var buffalo = ee.Image('projects/wri-datalab/SBTN/GLW4/GLW4-2020-D-DA-BFL');
var cattle  = ee.Image('projects/wri-datalab/SBTN/GLW4/GLW4-2020-D-DA-CTL');
var goats   = ee.Image('projects/wri-datalab/SBTN/GLW4/GLW4-2020-D-DA-GTS');
var sheep   = ee.Image('projects/wri-datalab/SBTN/GLW4/GLW4-2020-D-DA-SHP');

// Apply high density thresholds:
var highDensityLivestock = ee.ImageCollection([cattle.updateMask(cattle.gt(55.35)),goats.updateMask(goats.gt(119.63)),
                                        sheep.updateMask(sheep.gt(119.51)),buffalo.updateMask(buffalo.gt(51.86))]).mosaic();


var glwProj = cattle.projection();

// Reproject 10km GLW data to 30m: 
var livestock30 = highDensityLivestock.reproject({crs:umdProj}).selfMask();
// EXPORT THIS FOR FASTER PROCESSING ^
...
// Remove high density livestock from bare layer and classify remaining bare as natural: 
var bare_natural = bare.unmask().gt(0).updateMask(livestock30.gt(0).unmask().not());
...
Map.addLayer(intermediateMap, {min:2, max:16, palette: ["347834","B9B91E","6BAED6","06A285","FEFECC","ACD1E8",'ffffff','ffffff','ffffff','ffffff',"D3D3D3","D3D3D3","D3D3D3","D3D3D3","D3D3D3",]}, 'intermediateMap',0)
...
var nlcd_notNat = nlcd2019.remap([21,22,23,24,81,82],[13,13,13,13,15,12]).rename(['constant']).toUint8();
...
Map.addLayer(intermediateMap_wLocal, {min:2, max:16, palette: ["347834","B9B91E","6BAED6","06A285","FEFECC","ACD1E8",'ffffff','ffffff','ffffff','ffffff',"ded485","bd6c5b","bdbdbd","969696","737373",]}, 'intermediateMap_wLocal',0);
...
//// BARE MMU-------------------------------
// Count connected pixels:
var pixel_countB = natBare.connectedPixelCount({eightConnected:true}).reproject({crs:umdProj});
// Calculate pixel area: 
var pixelAreaB = ee.Image.pixelArea().reproject({crs:umdProj});
// Mutliply pixel count x area: 
var objectAreaB = pixel_countB.multiply(pixelAreaB);
// Set threshold (m^2) and apply:
var mmu = 5000;
var mmu_maskB = objectAreaB.gte(mmu);

// Create natural bare with mmu applied:
var bare_natmmu = natBare.updateMask(mmu_maskB).rename(['b1']);

// Create updated non-natural bare:
var bare_notNatmmu = bare.selfMask().updateMask(bare_natmmu.eq(1).unmask().not()).rename(['b1']);  

// Combine into one image for exporting 
var allBare_mmu = ee.ImageCollection([bare_notNatmmu.selfMask().multiply(2),bare_natmmu.selfMask()]).mosaic();
// EXPORT THIS FOR FASTER PROCESSING ^
...
var natural = natural.where(natural.eq(1).and(bare_natmmu.eq(1)), 6);       //bare = 6
...
var all = all.where(all.eq(1).and(bare_notNatmmu.eq(1)), 21);               //non natural bare = 21
...
/// re-apply SDPT & GLW where both SDPT/GLW & nat short veg (the filled in pixels)
...
var all = all.where(livestock30.gt(0).and(all.eq(3)), 15);                  //non natural short veg = 15 (apply GLW to nat short veg to re-label the filled in pixels)
...
// Example:
// exportImage(wcBareG,world,'wcBare_30m_reprojected_V1')
